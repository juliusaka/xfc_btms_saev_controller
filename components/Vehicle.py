...
class Vehicle:
    ...
    def update_energy_lag(self, t_act):
    ...
    def get_charging_trajectory_upper(self,t_act, timestep, N, maxPowerPlug):
        ...
        E0 = v.VehicleEngy
        for i in range(N):
            maxPower = v.get_max_charging_power(timestep, maxPowerPlug=maxPowerPlug)
            v.add_power(maxPower, timestep)
            traj.append(v.VehicleEngy - E0)
        del v
        return traj

    def get_charging_trajectories(self,t_act, timestep, N, maxPowerPlug):
        ...
        if t_act >= v.VehicleDesEnd:
            ...
            if k < N: # if prediction horizon is larger then desired charging duration
                ...
                for i in range(k): # start "de-charging" at k
                    ...
                    power = v.get_max_charging_power(timestep, inverse=True, maxPowerPlug=maxPowerPlug)
                    v.add_power( -1 * power, timestep)
