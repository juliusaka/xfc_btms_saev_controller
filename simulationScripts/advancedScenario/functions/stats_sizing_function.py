...
def stats_sizing_function_wait_time(df, timestep):
    # because E_BTMS is one entry longer than the other entries, the last entry is dropped with [:-2]
    btms_size = df['param: btms size, a,b_sys,b_cap,b_loan,c,d_wait_cost'].iloc[0]
    cost_a = df['param: btms size, a,b_sys,b_cap,b_loan,c,d_wait_cost'].iloc[1] * df['P_Grid'].max() 
    cost_b_sys = df['param: btms size, a,b_sys,b_cap,b_loan,c,d_wait_cost'].iloc[2] * df['P_BTMS'][:].abs().max()
    cost_b_cap = df['param: btms size, a,b_sys,b_cap,b_loan,c,d_wait_cost'].iloc[3] * sum(df['P_BTMS_Ch'][:-2].abs() * timestep/3600)
