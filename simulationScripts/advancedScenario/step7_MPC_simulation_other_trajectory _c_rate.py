...
path = result_parent_directory + os.sep + 'step7_MPC_simulation_other_charging_events_c_rate'
...
def main(chargingStationId, SOC_start, SOC_reference, N_mpc, rho_mpc, M1_mpc, use_events_of_charging_station):
    ...
    def simulation_for_one_charging_station(chargingStation: chargingStationClass):
        ...
        result_directory_one_simulation = os.path.join(result_directory, str(chargingStation.ChargingStationId) + '_events_of_' + str(use_events_of_charging_station))
...
if __name__ == '__main__':
    ...
    taz_list =['435', '435']
    use_events_of = ['18', '276']

    # make iterable for multiprocessing

    iterable = [[taz_list[i], SOC_start, SOC_reference, N, rho_mpc, M1_mpc, use_events_of[i] ]for i in range(len(taz_list))]
    if False:
